...
def generate_currency_report_regional(df_main_data, df_exclusion, df_pg, df_sbp, region_name, currency_type):
    ...
    # Rows with Scheme_Name containing Compute or Storage, Data Type 'Orders' and PRODUCT_LINE 'N3', 
    # to be added to compute_storage_filtered instead
    compute_storage_orders_n3 = df_main_data[
        df_main_data['Scheme_Name'].str.contains('Compute|Storage', case=False, na=False) & 
        (df_main_data['DATA_TYPE'] == 'Orders') & 
    ...
    compute_storage_unfiltered_orders_n3 = df_main_data_unfiltered[
        df_main_data_unfiltered['Scheme_Name'].str.contains('Compute|Storage', case=False, na=False) & 
        (df_main_data_unfiltered['DATA_TYPE'] == 'Orders') & 
    ...
    # LA Sales Exclusions
    # based on the DISTRIBUTOR_PARTY_ID
    rcs_exclusions_df = df_rcs.groupby('PRODUCT_LINE')[f'NET_TOTAL_{currency_type.upper()}'].sum().reset_index()    # need ton change this, along with PL it needs to check the ID as well.
    rcs_exclusions_df.rename(columns={f'NET_TOTAL_{currency_type.upper()}': 'LA Sales'}, inplace=True)
    df_report = df_report.merge(rcs_exclusions_df, on='PRODUCT_LINE', how='left')
...
def filter_and_export_disty_data(df_data, region_name, currency_col):
    ...
    compute_storage_orders_n3 = df_disty_filtered[
        df_disty_filtered['Scheme_Name'].str.contains('Compute|Storage', case=False, na=False) &
        (df_disty_filtered['DATA_TYPE'] == 'Orders') &
...
if all_sheets_from_new_file is not None:
    ...
    def update_partner_file_with_tcv(partner_id, tcv_summary, region_folder, currency_col):
        """Update partner file's Summary sheet with TCV amounts in the Net QTD Performance column and compute dependent columns"""
        ...
        try:
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                ...
                # Process Summary sheet if it exists
                if 'Summary' in existing_file:
                    ...
                    if len(partner_tcv_data) > 0:
                        print(f'[*] Updating Partner {int(partner_id)} with TCV data:')
                        for _, tcv_row in partner_tcv_data.iterrows():
                            scheme_name = tcv_row['Scheme Name']
                            tcv_amount = tcv_row[currency_col]
                            
                            # Find matching row in Summary sheet where New Name equals Scheme Name
                            mask = summary_df['New Name'] == scheme_name
                            if mask.any():
                                # Use .at for safer scalar access
                                idx = summary_df.index[mask][0]
                                
                                # Get current values and handle NaN by treating as 0
                                current_net_qtd = summary_df.at[idx, 'Net QTD Performance']
                                current_program_pct = summary_df.at[idx, 'Program %']
                                days_reporting = summary_df.at[idx, 'Days of Reporting']
                                days_quarter = summary_df.at[idx, 'Days in Quarter']
                                
                                if pd.isna(current_net_qtd):
                                    current_net_qtd = 0.0
                                if pd.isna(current_program_pct):
                                    current_program_pct = 0.0
                                if pd.isna(days_reporting) or days_reporting == 0:
                                    days_reporting = 1  # Avoid division by zero
                                if pd.isna(days_quarter):
                                    days_quarter = 0.0
                                
                                # Update Net QTD Performance by adding TCV amount
                                new_net_qtd = current_net_qtd + tcv_amount
                                summary_df.at[idx, 'Net QTD Performance'] = new_net_qtd
                                
                                # Compute dependent columns based on the updated Net QTD Performance
                                # Net Projected Performance = (Net QTD Performance / Days of Reporting) * Days in Quarter
                                new_net_projected = (new_net_qtd / days_reporting) * days_quarter
                                summary_df.at[idx, 'Net Projected Performance'] = new_net_projected
                                
                                # Amount = Net QTD Performance * Program %
                                new_amount = new_net_qtd * current_program_pct
                                summary_df.at[idx, 'Amount'] = new_amount
                                
                                # Total Rebate Amount = Amount (same as Amount)
                                summary_df.at[idx, 'Total Rebate Amount'] = new_amount
                                
                                # Projected Rebate Amount = (Amount / Days of Reporting) * Days in Quarter
                                new_projected_rebate = (new_amount / days_reporting) * days_quarter
                                summary_df.at[idx, 'Projected Rebate Amount'] = new_projected_rebate
                                
                                print(f'    - {scheme_name}: Added {tcv_amount:,.2f} to Net QTD Performance')
                                print(f'      Net QTD Performance: {current_net_qtd:,.2f} -> {new_net_qtd:,.2f}')
                                print(f'      Amount: {new_amount:,.2f}')
                                print(f'      Net Projected Performance: {new_net_projected:,.2f}')
                                print(f'      Projected Rebate Amount: {new_projected_rebate:,.2f}')
                            else:
                                print(f'    - No matching New Name found for: {scheme_name}')
